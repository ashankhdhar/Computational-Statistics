...
Now to transform everything into the original scale we will use the $\beta_{0}$ and $\beta{1}$ values to get the estimated values for $a$ and $x_{min}$.

```{r}
a.b1<- 1-beta.1
xmin.b0 <- exp(beta.0/(a.b1-1))
```

Now plotting to see the fit on the original scale
```{r}
...
# plot the data and the estimated functions as they were before
...
# curve after the tranform
curve(tail.prob(x, a.b1, xmin.b0), from = min(data.2012$income), to = max(data.2012$income),
add = T, col = "green")

# the legend
legend("topright", lty = c(1, 1), col = c("red", "blue", "green"), legend = c(paste("a=",
round(a.1, 1), ", xmin=", round(xmin.1, 1), sep = ""), paste("a=", round(a.2,
1), ", xmin=", round(xmin.2, 1), sep = ""), paste("a=", round(a.b1,
1), ", xmin=", round(xmin.b0, 1), sep = "")))
```
