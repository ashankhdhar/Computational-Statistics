...
substr(presidents,7,7)      # Gives the 7th element of every string in president. IF there is not 7th element then returns an empty string for that specific string.
```
...
strsplit (scarborough.fair, ",") # gives a space becase there is a space after the "," so reads it as a space
...
```{r}
paste(presidents, " (", 41:45, ")", sep="_") # default separator is space but you can specify it
...
#rep(2,11) gives 2, 11 times
#1:11 would give 1, 11 times
# rep(1:11,times=rep(2,11)) will give 1 , 2 11 times
```
...
```{r}
#makes 1 string instead of 5 different ones and separates the elements by ;
...
```{r,eval=FALSE}
# find the indices which contain that string
grep("God", al2)
grepl("God", al2)
#finds the line where God is
al2[grep("God", al2)]
```
...
# remember strsplit returns a list so this code take the first elements of that list
...
```{r,eval=FALSE}
wc <- table(al2.words)
# sorts the number of times a word appears 
